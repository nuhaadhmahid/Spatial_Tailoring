...
class Trace:
    ...
    def exert_boundary(lines_1, lines_2, border_nodes):
        ...
        # trimming at the boundary
        lines_1 = Trace.trimming_line(lines_1, boundary_lines, limits)
        lines_2 = Trace.trimming_line(lines_2, boundary_lines, limits)
    ...
    def create_lattice(self,field_type="SE", bool_plot = False):
        ...
        for key, lines in lines_set.items():
            ...
            # exerting boundary
            chordwise, spanwise, ribs, trailing_edge = Trace.exert_boundary(lines_1, lines_2, self.border_nodes_2D)
...
class Lattice:
    ...
    def __init__(
        ...
        trace_lines: Trace | None = None,
        grid_data: Trace | None = None,
    ...
    def create_fairing_2D(self, bool_plot=False):
        ...
        if self.lattice_lines is None:
            ...
            if os.path.exists(lattice_file):
                ...
                lattice = Trace(self.directory, self.reference_case)
                lattice.analysis()
        ...
        for group_name, group_lines in self.lattice_lines.items():
            ...
            # Refine lines            
            num_workers = min((os.cpu_count() - 2, len(group_lines)))    
            args = [(line, 0.02) for line in group_lines]
            with ThreadPoolExecutor(num_workers) as executor:
                group_lines = list(executor.map(lambda ab: Utils.GeoOps.increment_line(*ab), args))
            
            self.mesh2D.add_lines_to_mesh(0, group_lines, group_name)
    ...
    def write_mesh(self):
        ...
        # write to file
        with open(os.path.join(directory.case_folder, "mesh", f"{self.case_number}_fairing_geometry.inp"), "w", encoding="utf-8") as f:
            f.write("\n".join(lines) + "\n")
...
if __name__ == "__main__":
    ...
    # Trace Lattice
    lattice_data = Trace(directory, case_number)
